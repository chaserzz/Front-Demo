...
export default memo(function Index() {
  //state
  const [defaultClassName, setDefaultClassName] = useState("header-default-display"); //默认展示的h3的类名
  const [infoClassName, setInfoClassName] = useState("header-info-hidden")
  const [isMonseOver, setIsMonseOver] = useState(false);

  //other hooks
  useEffect(() => {
    //每次isMouseOver的改变都会触发
    handleHeaderDisplayChange(isMonseOver);
  }, [isMonseOver])
